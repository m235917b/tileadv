...
bool View::drawGame(const Chunk &chunk,
  ...
  for (int y = cameraY;
    ...
    for (int x = cameraX;
      ...
      float posX =
          leftMargin + static_cast<float>(x) * tileSize - cameraX * tileSize;
      float posY =
          topMargin + static_cast<float>(y) * tileSize - cameraY * tileSize;
  ...
  for (const auto character : characters) {
    ...
    float posY = topMargin +
                 static_cast<float>(character->getPosY()) * tileSize -
                 cameraY * tileSize;
